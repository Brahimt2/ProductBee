...
function featureToTask(
  ...
  const endDate = feature.endDate
    ...
    ? new Date(
        new Date(feature.startDate).getTime() +
          feature.duration * 24 * 60 * 60 * 1000
      )
  ...
  const dependencies: string[] = (feature.dependsOn || [])
    .map((depId) => {
      const depFeature = allFeatures.find(
        (f) => f._id === depId || f.id === depId
      );
  ...
  return {
    ...
    progress:
      feature.status === "complete"
        ? 100
        : feature.status === "in_progress" || feature.status === "active"
        ? 50
        : 0,
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-8 border border-gray-200 dark:border-gray-700">
        <div className="text-center text-gray-500 dark:text-gray-400">
          ...
          <p className="text-sm">
            Features need start dates, end dates, or durations to appear in the
            Gantt chart.
          </p>
        </div>
      </div>
